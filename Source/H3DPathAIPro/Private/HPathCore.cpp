...
void AHPathCore::AssignRequest(FS_PathRequest& Request)
{
    ...
    Async(EAsyncExecution::ThreadPool, [this, WorldRef, VolumeRef, Start, End, Delegate, bIsWalking, CharacterRadius, CharacterHalfHeight, OwnerRef]()
    {
        ...
        FindPath(WorldRef, VolumeRef, Result, Start, End, bIsWalking, CharacterRadius, CharacterHalfHeight, OwnerRef);
        
        AsyncTask(ENamedThreads::GameThread, [this, Result, Delegate]()
        {
            UE_LOG(LogTemp, Warning, TEXT("Path Found: %d"), Result->bSuccess);
...
void AHPathCore::FindPath(UWorld* WorldRef,AHVolume3D* VolumeRef,FS_PathResult* Result,FVector Start,FVector End,bool bIsWalking,float CharacterRadius,float CharacterHalfHeight,AActor* OwnerRef, int tryIndex)
{
    ...
    if (!IsLocationFree(WorldRef, End, CharacterRadius, CharacterHalfHeight, OwnerRef,false))
    {
    ...
    TArray<FVector> tryVectors;
    tryVectors.Add(FVector(0, 0, -1));
    tryVectors.Add(FVector(0, 0, 1));
    tryVectors.Add(FVector(0,0,-2));
    tryVectors.Add(FVector(0,0,2));
    tryVectors.Add(FVector(0, 0, -3));
    tryVectors.Add(FVector(0, 0, 3));
    tryVectors.Add(FVector(0,0,-4));
    tryVectors.Add(FVector(0,0,4));

    if (tryIndex >= tryVectors.Num())
    {
        Result->bSuccess = false;
        return;
    }
    FVector Direction = tryVectors[tryIndex];
    FVector NewEnd = OriginalEnd + Direction * 30.0f;
    UE_LOG(LogTemp, Warning, TEXT("Trying to find path again: %s"), *NewEnd.ToString());
    EndRef = NewEnd;
    Result->PathPoints.Empty();
    FindPath(WorldRef, VolumeRef, Result, Start, NewEnd, bIsWalking, CharacterRadius, CharacterHalfHeight, OwnerRef, tryIndex + 1);
}
    ...
    if (bIsWalking)
    {
        for (int32 i = 0; i < 4; i++)
        {
            ...
            if (NeighborGridID != -1)
            {
                ...
                if (IsLocationFree(WorldRef, NeighborPosition, CharacterRadius, CharacterHalfHeight, OwnerRef,false))
                {
                    ...
                    while (!foundAvailableGrid)
                    {
                        ...
                        if (downGridID == -1 || !IsLocationFree(WorldRef, downPosition, CharacterRadius, CharacterHalfHeight, OwnerRef,false))//try to find unavailable grid because we are trying to find ground
                        {
                            ...
                            if (upGridID != -1)
                            {
                                ...
                                else if (IsLocationFree(WorldRef, upPosition, CharacterRadius, CharacterHalfHeight, OwnerRef,false))
                                {
                                    FAStarNode NeighborNode;
                                    NeighborNode.GridID = upGridID;
                                    NeighborNode.WorldLocation = upPosition;
...
void AHPathCore::GetUpSideNeighbor(UWorld* WorldRef,AHVolume3D* VolumeRef,FVector neighborPosition,const FAStarNode& CurrentNode,TArray<FAStarNode>& OutNeighbors,TMap<int32, FAStarNode*>& OpenMap,TSet<int32>& ClosedSet,float CharacterRadius,float CharacterHalfHeight,AActor* OwnerRef)
{
    ...
    if (NeighborGridIDUpDown != -1)
    {
        ...
        if (IsLocationFree(WorldRef, NeighborPositionUpDown, CharacterRadius, CharacterHalfHeight, OwnerRef,false))
        {
            ...
            if (dot > 0.5f)
            {
                ...
                bool bLeft = !IsLocationFree(WorldRef, neighborLeft, CharacterRadius, CharacterHalfHeight, OwnerRef,false);
                bool bRight = !IsLocationFree(WorldRef, neighborRight, CharacterRadius, CharacterHalfHeight, OwnerRef,false);
                if (bLeft && bRight)
                {
                    FAStarNode NeighborNode;
...
bool AHPathCore::IsLocationFree(UWorld* WorldRef,const FVector& Location,float CharacterRadius,float CharacterHalfHeight,AActor* OwnerRef, bool bCameFromEndAdjustment)
{
    if (!WorldRef){return false;}

    FCollisionShape CollisionShape;
    if (!bCameFromEndAdjustment && (FVector::Dist(StartRef,Location) <= CharacterRadius || FVector::Dist(EndRef,Location) <= CharacterRadius))
    {
        CollisionShape = FCollisionShape::MakeCapsule(5, CharacterHalfHeight);
    }
    else
    {
         CollisionShape = FCollisionShape::MakeCapsule(CharacterRadius / 1.25, CharacterHalfHeight);
    }
...
// Check if the path can skip or not
bool AHPathCore::CanSkip(UWorld* WorldRef,FVector Start,FVector End,float CharacterRadius,float CharacterHalfHeight)
{
    ...
    FCollisionShape CollisionShape = FCollisionShape::MakeCapsule(CharacterRadius / 1.25 , CharacterHalfHeight);
    FCollisionQueryParams QueryParams;
    QueryParams.bTraceComplex = false;
...
{
    UE_LOG(LogTemp, Error, TEXT("Adjusting end position: %s"), *EndPosition.ToString());
    float gridSize = VolumeRef->CellSize;
    ...
    for(int a = 1; a < 5; a++)
    {
        for(int i = 0; i < SearchDirections.Num(); i++)
        {
            ...
            if(IsLocationFree(WorldRef,PotentialPosition, CharacterRadius, CharacterHalfHeight, OwnerRef, true))
            {
                UE_LOG(LogTemp, Warning, TEXT("Adjusted end position: %s"), *PotentialPosition.ToString());
                EndRef = PotentialPosition;
